...
const player = { w:34, h:44, x:40, y:GROUND_Y, vx:0, vy:0, speed:320, jump:820, onGround:true };
...
function buildLevel(){
  ...
  const items = [
    ...
    {x:340,y:GROUND_Y,w:60,h:140,type:'block'},
    {x:470,y:GROUND_Y,w:70,h:100,type:'pipe'},
    {x:620,y:GROUND_Y,w:70,h:160,type:'block'},
    {x:700,y:GROUND_Y,w:70,h:200,type:'block'},
    {x:780,y:GROUND_Y,w:70,h:240,type:'block'},
    {x:920,y:GROUND_Y,w:70,h:120,type:'pipe'},
    {x:1040,y:GROUND_Y,w:70,h:80,type:'pipe'},
    {x:1160,y:GROUND_Y,w:70,h:150,type:'pipe'},
    {x:1260,y:GROUND_Y,w:140,h:60,type:'block'},
    {x:1420,y:GROUND_Y,w:60,h:180,type:'block'},
    {x:1520,y:GROUND_Y,w:60,h:220,type:'block'},
    {x:1600,y:GROUND_Y,w:120,h:60,type:'block'},
    {x:1740,y:GROUND_Y,w:60,h:200,type:'block'}
  ];
  // Scale block 70%
  items.forEach(o => { if(o.type==='block'){ o.w*=0.7; o.h*=0.7; } });

  // Pohon dekoratif
  items.push({ x: 1850, y: GROUND_Y, w: 50, h: 70, type: 'tree-trunk' });
  items.push({ x: 1810, y: GROUND_Y + 70, w: 130, h: 110, type: 'tree-leaf' });
  ...
  giftEl.style.bottom = (GROUND_Y + 170) + "px";
...
function centerCamera(){
  ...
  let offset = 100; // default PC

  if (window.innerWidth < 768) {
    offset = 180; 
    worldEl.style.transformOrigin = "top left";
    worldEl.style.scale = "0.80"; // sudah oke zoomnya
  } else {
    worldEl.style.transformOrigin = "";
    worldEl.style.scale = "1";
  }

  let camX = player.x - viewW / 2 + player.w / 2 - offset;
  camX = Math.max(0, Math.min(camX, WORLD_WIDTH - viewW));

  // ✅ TARUH DI SINI — kamera geser naik dikit biar arena nggak terlalu bawah
  let transformStyle = `translateX(${-camX}px)`;
  if (window.innerWidth < 768) {
    transformStyle += " translateY(-25px)"; // angka 20–30px ideal
  }

  worldEl.style.transform = transformStyle;
}
